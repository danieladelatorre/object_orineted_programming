...
namespace coen79_lab9
{
    ...
    template <class Process, class BTNode>
    void inorder(Process f, BTNode* node_ptr)	//goes in the order left, root, right
    ...
    template <class Process, class BTNode>
    void postorder(Process f, BTNode* node_ptr)	//goes in the order left, right, root
    // Library facilities used: cstdlib
    {
    ...
    template <class Process, class BTNode>
    void preorder(Process f, BTNode* node_ptr)	//goes in the order of root, left, right
    // Library facilities used: cstdlib
    {
        f(node_ptr->data());
    ...
    template <class Item>
    void tree_clear(binary_tree_node<Item>*& root_ptr)  //must delete in postorder
    // Library facilities used: cstdlib
    {
        binary_tree_node<Item>* child;
        if (root_ptr != NULL)
        {
            child = root_ptr->left( );	//deletes the left node
            tree_clear( child );
            child = root_ptr->right();	//deletes the right node
            tree_clear(child);
            delete root_ptr;
            root_ptr = NULL;
            // STUDENT WORK
    ...
    template <class Item>
    binary_tree_node<Item>* tree_copy(const binary_tree_node<Item>* root_ptr)   //im doing this in postorder
    // Library facilities used: cstdlib
    {
        binary_tree_node<Item> *l_ptr;
        binary_tree_node<Item> *r_ptr;
        binary_tree_node<Item> *new_root_ptr;
        
        if (root_ptr == NULL)
            return NULL;
        else    //root pointer of other tree is not NULL
        {
            l_ptr = tree_copy(root_ptr->left());
            r_ptr = tree_copy(root_ptr->right());
            new_root_ptr = new binary_tree_node<Item>(root_ptr->data(), l_ptr, r_ptr);	//makes a new root node from the left and right subtrees
            // STUDENT WORK
        }
        return new_root_ptr;
    }
    ...
    // Library facilities used: cstdlib
    {
        if (node_ptr == NULL)
            ...
            return (1 + tree_size(node_ptr->left()) + tree_size(node_ptr->right()));	//adds 1 each time it reaches a nonempty node
            // STUDENT WORK
    }
}
