...
#pragma mark - Toolkit Function Implementation
    ...
    // to a copy of the removed item.
    {
        ...
        if (root_ptr->right( ) != NULL)
            bst_remove_max(root_ptr->right( ), removed);	//keep going until we find the next largest node
        else	//root_ptr is the next biggest item
        {
        	removed = root_ptr->data();	//data from next largest is copied over to removed
        	oldroot_ptr = root_ptr;		//saves the root pointer so we can remove it
        	root_ptr = root_ptr->left();	//root pointer is set to the left child (can have value or be null either works)
        	delete oldroot_ptr;		//deletes the old root pointer
            // STUDENT WORK
        }
    }
...
#pragma mark - Member Function Implementation
    ...
    {
        tree_clear(root_ptr);	//delete all nodes in the tree
    }
    ...
    // Header file used: bintree.h
    {
        ...
        do
        {
            ...
            {   // Go left
                if(cursor->left()==NULL){	//if the left node is null we add the new entry there and stop the loop
                	done = true;
            ...
            {   // Go right
            	if(cursor->right()==NULL){	//if the right node is null we enter the new entry there and stop the loop
            		done = true;
    ...
    typename bag<Item>::size_type bag<Item>::count(const Item& target) const
    {
        ...
        while (cursor != NULL)
        {
            ...
            {	
		if(cursor->data()==target){	//adds one every time we find the target
			answer++;
		}
                // STUDENT WORK
		cursor = cursor->left();
    ...
    // Header file used: bintree.h
    {
    	...
    	tree_clear(root_ptr);	//must clear the actiavted tree because = operator is the same as copying to an empty tree
    	root_ptr = tree_copy(source.root_ptr);
    	return;
        // STUDENT WORK
    }
    ...
    // method.
    {
        if (addroot_ptr != NULL)
        {
        	insert(addroot_ptr->data());	//adds the nodes in preorder
